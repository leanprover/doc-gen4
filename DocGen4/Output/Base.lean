...
def moduleToHtmlLink (module : Name) : BaseHtmlM Html := do
  return html%{<a href={← moduleNameToLink module}>{module.toString}</a>}
...
def declNameToHtmlLink (name : Name) : HtmlM Html := do
  return html%{<a href={← declNameToLink name}>{name.toString}</a>}
...
def breakWithin (name: String) : (Array Html) :=
  name.splitOn "."
    |> .map (fun (s: String) => html%{<span class="name">{s}</span>})
    |> .intersperse "."
    |> List.toArray
...
def declNameToHtmlBreakWithinLink (name : Name) : HtmlM Html := do
  return html%{<a class="break_within" href={← declNameToLink name}>
    ...
    | .const name =>
      ...
      if name2ModIdx.contains name && (Lean.privatePrefix? name).isNone then
        ...
        if innerHasAnchor then
          ...
          return (true, #[html%{<a href={link}>{innerHtml}</a>}])
      else
...
where
  fn (html : Array Html) : Array Html := #[html%{<span class="fn">{html}</span>}]
...
def baseHtmlHeadDeclarations : BaseHtmlM (Array Html) := do
  return #[
    ...
    html%{<link rel="stylesheet" href={s!"{← getRoot}style.css"}/>},
    html%{<link rel="icon" href={s!"{← getRoot}favicon.svg"}/>},
    html%{<link rel="mask-icon" href={s!"{← getRoot}favicon.svg"} color="#000000"/>},
    html%{<link rel="prefetch" href={s!"{← getRoot}/declarations/declaration-data.bmp"} as="image"/>}
  ]
