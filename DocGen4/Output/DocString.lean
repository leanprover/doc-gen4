...
/--
Check if a name has an HTML anchor in its module's documentation.
Names get anchors if they are rendered members, constructors, direct fields,
or parent projections.
-/
private def hasHtmlAnchor (res : AnalyzerResult) (name : Name) : Bool :=
  if let some modIdx := res.name2ModIdx[name]? then
    let modName := res.moduleNames[modIdx.toNat]!
    if let some mod := res.moduleInfo[modName]? then
      mod.members.any fun
        | .docInfo d =>
          -- Rendered declarations get <div id="name">
          if d.getName == name && d.shouldRender then true
          -- Constructors get <ul id="name"> even though shouldRender is false
          else if d.getName == name then
            match d with
            | .ctorInfo _ => true
            | _ => false
          else
            match d with
            -- Structure/class fields: direct fields get <li id="name">
            | .structureInfo s | .classInfo s =>
              s.fieldInfo.any fun f => f.name == name && f.isDirect ||
              s.parents.any (·.projFn == name)  -- parent projections get <span id="name">
            | _ => false
        | .modDoc _ => false
    else false
  else false
  ...
  if s.endsWith ".lean" && s.contains '/' then
    ...
    if res.name2ModIdx.contains name && !isPrivateName name then
      ...
      if let .str parent suffix := name then
        if isAutoGeneratedSuffix suffix then
          if res.name2ModIdx.contains parent && !isPrivateName parent then
            return ← declNameToLink parent
          else return none
      -- Only link if the name actually has an HTML anchor
      if hasHtmlAnchor res name then
        declNameToLink name
      else
        return none
    -- module name
